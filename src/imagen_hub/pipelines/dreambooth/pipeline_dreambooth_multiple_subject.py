...
from imagen_hub.pipelines.dreambooth.utils.dataset import MultiTrainDataset
...
class DreamBoothPipelineMulti:
    def __init__(self, device="cuda",
                 subject_names = ["dog", "barn"],
                 identifiers = ["<new1>", "<new2>"],
                 data_path = None,
                 evaluate_prompt = "photo of a sleeping <new1> dog in front of a <new2> barn",
                 num_epochs: int = 1,
        ...
        self.instance_prompts = ["photo of a " + identifier + " " + subject_name for identifier, subject_name in zip(identifiers, subject_names)]
        self.class_prompts = ["photo of a " + subject_name for subject_name in subject_names]
    ...
    def retrieve(self):
        ...
        for s, class_prompt in enumerate(self.class_prompts):
            results = client.query(text=class_prompt)
            logger.info("{} of {} are retrieved".format(len(results), class_prompt))
            save_folder = os.path.join(self.data_path, "class", self.subject_names[s])
            if os.path.exists(save_folder) and abs(len(results) - len(os.listdir(save_folder))) <= 20:
                print('using retrieved images')
                continue
            ...
            for i, result in enumerate(results):
                if not result["url"].endswith(".jpg"):
                    continue
                try:
                    urllib.request.urlretrieve(result["url"], os.path.join(save_folder,  "found" + str(i) + ".jpg"))
                except:
                    # logger.info("failure")
                    continue
    ...
    def evaluate_one(self, evaluate_prompt, pipeline: StableDiffusionPipeline):
        # Sample some images from random noise (this is the backward diffusion process).
        # The default pipeline output type is `List[PIL.Image]`
        with torch.no_grad():
            with torch.autocast("cuda"):
    ...
    def get_dataloader(self,):
        dataset = MultiTrainDataset(self.data_path, self.subject_names, self.instance_prompts, self.class_prompts, self.image_size)
    ...
    def train(self,):
        ...
        model = StableDiffusionPipeline.from_pretrained(self.model_id, use_auth_token=True).to(self.device)

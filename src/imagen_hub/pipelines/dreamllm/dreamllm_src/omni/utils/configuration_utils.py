...
class ConfigMixin:
    ...
    Also provides the [`~ConfigMixin.from_config`] and [`~ConfigMixin.save_config`] methods for loading, downloading, and
    ...
    Class attributes:
        ...
        - **_deprecated_kwargs** (`List[str]`) -- Keyword arguments that are deprecated. Note that the `init` function
          should only have a `kwargs` argument if at least one argument is deprecated (should be overridden by subclass).
    """
    ...
    @classmethod
    def from_config(cls, config: Union[FrozenDict, dict[str], Any] | None = None, return_unused_kwargs=False, **kwargs):
        r"""
    ...
    def extract_init_dict(cls, config_dict, **kwargs):
        ...
        omni_library = importlib.import_module(__name__.split(".")[0])

        if cls.has_compatibles:
            compatible_classes = [c for c in cls._get_compatibles()]
        else:
            compatible_classes = []
        ...
        if isinstance(orig_cls_name, str) and orig_cls_name != cls.__name__ and hasattr(omni_library, orig_cls_name):
            orig_cls = getattr(omni_library, orig_cls_name)
    ...
    def to_json_string(self) -> str:
        ...
        config_dict["_omni_version"] = __version__
...
def register_to_config(init):
    ...
    def inner_init(self, *args, **kwargs):
        ...
        # Then add all kwargs
        new_kwargs.update(
            {k: init_kwargs.get(k, default) for k, default in parameters.items() if k not in ignore and k not in new_kwargs}
        )

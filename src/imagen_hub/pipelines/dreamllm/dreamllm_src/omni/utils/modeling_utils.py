...
def get_model_requires_grad(model: Union[torch.nn.Module, torch.Tensor], *, check_entire: bool = False):
    # BUG: Model maybe partially requires_grad
    if not isinstance(model, (torch.nn.Module, torch.Tensor)):
        raise TypeError(f"`model` must be a `torch.nn.Module` or `torch.Tensor`, got `{type(model)}`")

    try:
        if isinstance(model, torch.nn.Module):
            parameters_and_buffers = itertools.chain(model.parameters(), model.buffers())
        else:
            parameters_and_buffers = itertools.chain(model)

        if check_entire:
            # As long as any parameter or buffer requires gradient, it will return True.
            return any(param.requires_grad for param in parameters_and_buffers)
        else:
            return next(parameters_and_buffers).requires_grad
        ...
        return first_tuple[1].requires_grad
        ...
        try:
            with open(checkpoint_file) as f:
                if f.read().startswith("version"):
                    ...
                    raise ValueError(
                        f"Unable to locate the file {checkpoint_file} which is necessary to load this pretrained model. "
    ...
    for param_name, param in state_dict.items():
        ...
        if empty_state_dict[param_name].shape != param.shape:
            ...
            raise ValueError(
                ...
                f"If you want to instead overwrite randomly initialized weights, please make sure to pass both `low_cpu_mem_usage=False` and `ignore_mismatched_sizes=True`. "
                f"For more information, see also: https://github.com/huggingface/diffusers/issues/1619#issuecomment-1345604389 as an example."
            )
...
""" ModelMixin base class and utilities."""
...
class ModelMixin(torch.nn.Module):
    ...
    _automatically_saved_args = ["_omni_version", "_class_name", "_name_or_path"]
    ...
    def set_use_memory_efficient_attention_xformers(self, valid: bool, attention_op: Union[Callable, None]= None) -> None:
    ...
    def enable_xformers_memory_efficient_attention(self, attention_op: Union[Callable, None]= None) -> None:
        r"""
    ...
    def save_pretrained(self, save_directory: Union[str, os.PathLike], is_main_process: bool = True, safe_serialization: bool = True):
        """
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: Union[None,str, os.PathLike], **kwargs):
        r"""
        ...
        if device_map is not None and not is_torch_available(">=", "1.9.0"):
            raise NotImplementedError(
                "Loading and dispatching requires torch >= 1.9.0. Please either update your PyTorch version or set"
                " `device_map=None`."
            )

        if low_cpu_mem_usage is True and not is_torch_available(">=", "1.9.0"):
            raise NotImplementedError(

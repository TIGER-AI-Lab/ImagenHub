...
def get_checkpoint_path(resume_path: str) -> str:
    # If given a direct .bin file, use its parent dir
    if resume_path.endswith(".bin") and os.path.isfile(resume_path):
        return os.path.dirname(resume_path)
    # If it's a folder, check it exists
    if os.path.isdir(resume_path):
        return resume_path
    raise FileNotFoundError(f"Could not resolve checkpoint path: {resume_path}")

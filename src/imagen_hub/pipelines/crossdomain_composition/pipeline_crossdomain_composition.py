...
CDC_DIR = 'src/imagen_hub/pipelines/crossdomain_composition/crossdomain_composition_src/'
sys.path.append(os.path.join(sys.path[1], CDC_DIR))
CDC_DIR = os.path.join(sys.path[1], CDC_DIR)
# print(CDC_DIR)
# print(sys.path)

from ldm.models.diffusion.ddim import DDIMSampler
from .crossdomain_composition_src.scripts_cdc.img2img_utils import create_argparser, chunk, load_img, load_mask, load_model_from_config, Preprocess
from .crossdomain_composition_src.cdc.prompt_masking import masked_cross_attention
import requests
# from huggingface_hub import hf_hub_download


def load_checkpoints():
    url1 = 'https://huggingface.co/CompVis/stable-diffusion-v-1-4-original/resolve/main/sd-v1-4.ckpt'
    ckpt1 = 'sd-v1-4.ckpt'
    url2 = 'https://huggingface.co/runwayml/stable-diffusion-inpainting/resolve/main/sd-v1-5-inpainting.ckpt'
    ckpt2 = 'sd-v1-5-inpainting.ckpt'
    dir = os.path.join(CDC_DIR, 'models/ldm/stable-diffusion-v1/')
    if not os.path.exists(os.path.join(dir, ckpt1)):
        print(f'Downloading checkpoint from {url1} to {dir}...')
        # hf_hub_download(repo_id="CompVis/stable-diffusion-v-1-4-original/resolve/main/", filename="sd-v1-4.ckpt", cache_dir=dir)
        resp = requests.get(url1)
        with open(os.path.join(dir, ckpt1), "wb") as f:
            f.write(resp.content)
    if not os.path.exists(dir + ckpt2):
        print(f'Downloading checkpoint from {url2} to {dir}...')
        resp = requests.get(url2)
        with open(os.path.join(dir, ckpt2), "wb") as f:
            f.write(resp.content)
...
class CrossDomainCompositionPipeline():
    ...
    @torch.no_grad()
    def __call__(self,
                 ckpt= os.path.join(CDC_DIR, 'models/ldm/stable-diffusion-v1/sd-v1-4.ckpt'),
                 config= os.path.join(CDC_DIR, "configs/stable-diffusion/v1-inference.yaml"),
                 init_img= os.path.join(CDC_DIR, 'examples/scribbles/images/corgi1.png'), 
                 mask=os.path.join(CDC_DIR, 'examples/scribbles/masks/corgi1.png'), 
                 prompt='a corgi wearing a business suit',
                 ddim_steps=50, T_out=1.0, T_in=0.8, down_N_out=1, down_N_in=1, repaint_start=0.4) -> PIL.Image.Image:
        ...
        opt.T_out = T_out
        opt.T_in = T_in
        opt.down_N_out = down_N_out
        opt.down_N_in = down_N_in
        opt.repaint_start = repaint_start
        # print(opt)
                    ...
                    for img, mask, prompts, filename in tqdm(data_loader, desc="Data"):
                        ...
                        for n in trange(sampling_conf.n_samples, desc="Sampling"):
                            ...
                            for i, x_sample in enumerate(x_samples):
                                ...
                                # Image.fromarray(x_sample.astype(np.uint8)).save(os.path.join(sample_path, f"{filename[i]}_{n:02}.jpg"))
